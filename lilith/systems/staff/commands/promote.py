...
async def promote(ctx, member: discord.Member):
    ...
    if current_level == 1:
        await ctx.send(
            f"❌ {member.mention} is already the highest rank."
        )
        return
    ...
    if new_rank is None:
        await ctx.send(
            "❌ The next higher staff rank could not be found."
        )
        return
    ...
    await ctx.send(
        f"⬆️ Promoted {member.mention} to **{new_rank[1]}**."
    )
